 ...
 * - 업로드 진행 상황 추적 및 동기화
 ************************************************************/
// 출처: practice07/file_up_thread_server.c, practice07/chat_serv.c의 include 패턴 참조
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <arpa/inet.h>
#include <pthread.h>  // practice07/file_up_thread_server.c의 7라인 참조
#include <sys/stat.h> // mkdir 사용을 위해 추가

// 출처: practice03/file_up_server.c의 BUF_SIZE 패턴 참조
 ...
 */
// 출처: practice06/echo_client.c의 48-54라인 패턴을 직접 참조한 함수
//       practice 코드에서 recv/read가 일부만 받을 수 있는 경우를 반복문으로 처리하는 패턴을 그대로 적용
int recv_exact(int sd, char *buf, int size)
{
    int received = 0;
    while (received < size)
    { // practice06/echo_client.c의 while (recv_len<str_len) 패턴 참조
        int n = recv(sd, buf + received, size - received, 0);
        if (n <= 0)
            return -1; // 수신 실패 또는 연결 종료
        received += n; // 수신된 바이트 수 누적 (practice06/echo_client.c의 recv_len += recv_cnt 패턴 참조)
    }
    return received;
}
 ...
 * @return: 성공 시 0
 */
// 출처: practice08/web_server.c의 파일 크기 확인 패턴 (117-120라인) 참조
int handle_FIRST(UploadSession *s, char *id, char *file, long filesize)
{
    ...
    // 출처: practice08/web_server.c의 117-120라인 (파일 크기 확인 패턴) 참조
 ...
 */
// 출처: practice03/file_up_server.c의 파일 수신 패턴 참조
int handle_DATA(UploadSession *s, int chunkSize)
{
    ...
    // 파일에 데이터 쓰기 (practice03/file_up_server.c의 파일 쓰기 패턴 참조)
    fwrite(buf, 1, chunkSize, s->fp);
    fflush(s->fp); // 디스크에 즉시 쓰기 (데이터 손실 방지) (practice08/web_server.c의 132라인 참조)
 ...
 * @return: NULL (스레드 종료)
 */
// 출처: practice07/chat_serv.c의 handle_clnt 함수 (62-88라인) 패턴 참조
//       practice07/file_up_thread_server.c의 handle_client 함수 패턴 참조
void *handle_client(void *arg)
{
    // 클라이언트 소켓 디스크립터 가져오기 (practice07/chat_serv.c의 64라인 참조)
    int sd = *(int *)arg;
    free(arg); // 동적 할당된 소켓 포인터 해제 (practice07/chat_serv.c의 스레드 인자 처리 패턴 참조)
    ...
    // 클라이언트와 통신하는 메인 루프 (practice07/chat_serv.c의 68라인 while 패턴 참조)
    while (1)
    {
        // 클라이언트로부터 한 줄 메시지 수신
    ...
    // 클라이언트 소켓 닫기 (practice07/chat_serv.c의 86라인 참조)
    close(sd);
    return NULL;
}
 ...
 */
// 출처: practice01/tcp_server.c의 main 함수 구조 참조
//       practice07/file_up_thread_server.c의 main 함수 (14-52라인) 참조
//       practice08/web_server.c의 SO_REUSEADDR 설정 (37-39라인) 참조
int main(int argc, char *argv[])
{
    // 명령행 인자 개수 확인 (practice01/tcp_server.c의 22-25라인 참조)
    if (argc != 2)
    {
        printf("Usage: %s <port>\n", argv[0]);
        exit(1);
    ...
    // TCP 소켓 생성 (practice01/tcp_server.c의 27라인 참조)
    int serv_sd = socket(PF_INET, SOCK_STREAM, 0);

    // 서버 재시작 시 TIME_WAIT로 bind 실패하는 것 방지
    // 출처: practice08/web_server.c의 37-39라인 참조
    int option = 1;
    ...
    // 서버 주소 구조체 설정 (practice01/tcp_server.c의 31-34라인 참조)
    struct sockaddr_in serv, clnt;
    memset(&serv, 0, sizeof(serv));

    serv.sin_family = AF_INET;                // IPv4
    serv.sin_addr.s_addr = htonl(INADDR_ANY); // 모든 네트워크 인터페이스에서 수신 (practice01/tcp_server.c의 33라인 참조)
    serv.sin_port = htons(atoi(argv[1]));     // 포트 번호를 네트워크 바이트 순서로 변환 (practice01/tcp_server.c의 34라인 참조)

    // 소켓을 주소에 바인딩 (practice01/tcp_server.c의 36라인 참조)
    if (bind(serv_sd, (struct sockaddr *)&serv, sizeof(serv)) < 0)
    {
        perror("bind");
        exit(1);
    }
    // 클라이언트 연결 대기 (최대 10개까지 대기 큐에 저장) (practice01/tcp_server.c의 39라인 참조)
    ...
    // 클라이언트 연결을 무한히 받는 메인 루프 (practice07/file_up_thread_server.c의 39라인 참조)
    while (1)
    {
        socklen_t sz = sizeof(clnt);
        // 클라이언트 연결 수락 (블로킹 함수) (practice07/file_up_thread_server.c의 42라인 참조)
        ...
        // 출처: practice07/chat_serv.c의 54라인 pthread_create 패턴 참조
        //       practice07/file_up_thread_server.c의 스레드 처리 패턴 참조
        // clnt_sd는 루프 변수라 스레드 시작 전에 별도 메모리에 복사
        ...
        *pclient = clnt_sd;
        ...
        pthread_t t;
        pthread_create(&t, NULL, handle_client, pclient); // practice07/chat_serv.c의 54라인 참조
        pthread_detach(t);                                // 스레드가 종료되면 자동으로 정리 (practice07/chat_serv.c의 55라인 참조)

        printf("Connected: %s\n", inet_ntoa(clnt.sin_addr)); // practice07/chat_serv.c의 56라인 참조
    }
